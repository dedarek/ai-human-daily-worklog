...
async function saveDraft(draft: DailyDraft) {
  ...
  await writeFile(join(draftsDir, `${draft.date}.json`), JSON.stringify(draft, null, 2), { mode: 0o600 });
  return draft;
}
...
async function generateDailyDraft(date: string, answers: Record<string, string> = {}): Promise<DailyDraft> {
  ...
  return saveDraft({ date, status: "draft", version: 1, createdAt: now, updatedAt: now, originalReport: report, editedReport: report, graph, trace, gaps, answers, events: activities.length, manifest });
}
...
export function createDailyDraft(date: string, force = false, answers: Record<string, string> = {}) {
  return runReport(async () => {
    ...
    return generateDailyDraft(date, answers);
  });
}
...
async function publishDraft(draft: DailyDraft) {
  ...
  const doc = await publishReport(draft.date, dailyXml(report, { date: draft.date, title: dailyTitle(draft.date), sourceSummary }), settings, secrets, publishedIndex[draft.date]?.documentId);
  const reportDir = join(dataDir, "reports"); await mkdir(reportDir, { recursive: true });
  await writeFile(join(reportDir, `${draft.date}.md`), report, { mode: 0o600 });
  const markdownPath = await writeMarkdownExport(`daily-${draft.date}.md`, report, settings);
  await updateJson<PublishedIndex>(publishedFile, {}, index => ({ ...index, [draft.date]: { date: draft.date, events: draft.events, ...doc } }));
...
export function run(date: string, force = false) {
  return runReport(async () => {
    ...
    return publishDraft(await generateDailyDraft(date));
  });
}
...
export function runSummary(kind: "weekly" | "monthly", start: string, end: string, force = true) {
  return runReport(async () => {
    ...
    const doc = await publishReport(start, summaryXml(report, { label, title }), settings, secrets, publishedIndex[key]?.documentId, kind, title);
    const reportDir = join(dataDir, "reports"); await mkdir(reportDir, { recursive: true });
