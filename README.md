...
1. **先建立可信度**：完成 Windows/Linux 实机验收，补健康检查、诊断导出、失败通知和安全升级。
2. **再建立项目理解**：自动识别项目边界，连接“意图 → 执行 → 产物 → 验证”，跨 Agent 去重。
3. **让结论可追溯**：报告句子可以回看本地证据和置信度，证据不足时不编造。
4. **加入发布前校正**：预览、项目重分配、局部重写、忽略和版本对比，并在本地学习用户修正。
5. **让档案开始复利**：生成晨间续接、项目状态、季度回顾，并支持带证据的历史搜索与问答。
...
- [ ] 连接用户意图、Agent 执行、代码/文档产物与最终验证，形成可查询的工作链路
- [ ] 将 Git commit、PR、Release、文档、设计和部署识别为一等工作产物
...
- [ ] 报告句子可回溯到本地证据与置信度，低置信结论在发布前提示
- [ ] 从用户的项目重分配、删除和改写中学习本地偏好，不上传个人行为画像
- [ ] 下班前对证据缺口提出至多 1–2 个聚焦问题，避免错误永久归档
- [ ] 生成晨间续接摘要，恢复活跃项目、未决事项、承诺和下一步动作
- [ ] 对个人历史档案进行带证据的搜索与问答，支持项目复盘和季度回顾
